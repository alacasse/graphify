...
- Fix: docs-only `graphify extract --no-cluster` incremental runs now compare semantic `source_file` values in normalized root-relative and absolute forms before stamping `semantic_hash`, so successful LLM output no longer leaves unchanged docs queued forever. The raw no-cluster writer also preserves existing graph entries for unchanged semantic files and replaces only changed/deleted sources before deduping, preventing partial semantic refreshes from shrinking `graph.json` to just the changed document set (#11, #15).
