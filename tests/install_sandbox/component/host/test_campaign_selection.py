...
from tests.install_sandbox.component.host.campaign_support import commands
from tools.install_sandbox.host.coordinator import InstallTestCoordinator

_ROOT = Path(__file__).resolve().parents[4]
_COMPONENT = Path(__file__).parent
_DEFAULTS = _ROOT / "tools/install_sandbox/host/campaign-defaults.yaml"
...
def arrange(root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (root / "bin").mkdir()
    (root / "bin/docker").symlink_to(_COMPONENT.parent / "host/fake_docker.py")
...
def test_copied_configuration_refusal_and_explicit_bypass(
    ...
    path = copy / "tools/install_sandbox/host/campaign-defaults.yaml"
    contents = {
        "syntax": "[",
        "key": "wrong: []",
        "empty": "default_cases: []",
...
def test_yaml_alone_changes_default_cases(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    original = _DEFAULTS.read_bytes()
    copy = sandbox_copy(tmp_path)
    (copy / "tools/install_sandbox/host/campaign-defaults.yaml").write_text(
...
def test_failed_report_write_keeps_cleanup_and_returns_campaign_failure(
    ...
    adapter = _COMPONENT.parent / "container/controlled_backup_case.py"
    wrapper.write_text(
        "import runpy, sys\nfrom pathlib import Path\n"
