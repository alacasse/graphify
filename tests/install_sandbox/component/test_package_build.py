...
_CONTAINERFILE = Path(__file__).resolve().parents[3] / "tools/install_sandbox/Containerfile"
...
def test_dependency_inputs_precede_candidate_and_runtime_sources() -> None:
    ...
    runtime = next(
        i for i, line in enumerate(lines) if "COPY --chmod" in line and "case.py" in line
    )
    assert metadata < dependencies < sources < package < runtime
    assert not any("subject" in line or "case.py" in line for line in lines[:dependencies])
    assert any(
