...
from tests.install_sandbox.component.test_observations_verifier import assert_passed, scenario
from tools.install_sandbox.case import InstallTestCase
from tools.install_sandbox.spec import HookExpectation, InstallTestSpec
...
def test_hook_spec_rejects_non_transportable_or_incomplete_facts(hooks: object) -> None:
    data = json.loads((Path(__file__).parent / "fixtures/first-install.json").read_text())["spec"]
...
def test_case_witness_survives_transport() -> None:
    case = InstallTestCase.from_json(
        (Path(__file__).parent / "fixtures/first-install.json").read_text()
    )
