...
MARKDOWN = ".sandbox-reference/instructions.md"
PREFIX = "# My project\n\n## Language\nRespond in English.\n\n"
SUFFIX = "## Changes\nExplain proposed changes.\n"
SECTION = "## graphify\nUse the graph.\n"
CONFORMING = PREFIX + SECTION + "\n" + SUFFIX
ALTERED = PREFIX + "## graphify\nSandbox altered Markdown section.\n\n" + SUFFIX


def _script(mode: str) -> str:
    install = controlled_script("passed")
    repaired = install + f"Path({MARKDOWN!r}).write_text({CONFORMING!r})\n"
    second = {
        "absent": "pass",
        "lost_prefix": repaired + f"Path({MARKDOWN!r}).write_text({(SECTION + SUFFIX)!r})",
        "lost_suffix": repaired + f"Path({MARKDOWN!r}).write_text({(PREFIX + SECTION)!r})",
        "wrong_section": repaired + f"Path({MARKDOWN!r}).write_text({ALTERED!r})",
        "missing": repaired + f"Path({MARKDOWN!r}).unlink()",
    ...
    if mode == "first_loss":
        first += f"Path({MARKDOWN!r}).write_text({(PREFIX + SECTION)!r})"
    if mode == "not_started":
...
def arrange_markdown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, mode: str = "passed") -> None:
    LocalCase(tmp_path)
    ...
    monkeypatch.setenv(
        "FAKE_DOCKER_CASE_PROGRAM", str(_COMPONENT / "controlled_markdown_repair_case.py")
    )
    monkeypatch.setenv("CONTROLLED_INSTALLER", str(executable))


def run_markdown(tmp_path: Path) -> CoordinatedResult:
    campaign = InstallTestCoordinator().run_campaign(
        specs_directory=_SPECS,
        target="sandbox-reference",
        case_names=["repair-markdown-section"],
