...
def test_universal_uninstall_derives_expected_keys_through_installer_core(oracle) -> None:
    ...
    class RecordingOracle(file_effect_oracle.FileEffectOracle):
        def __init__(self, wrapped: file_effect_oracle.FileEffectOracle) -> None:
            ...
            object.__setattr__(self, "generated_decisions", [])
        ...
        def generated_file_decision(
            ...
            *,
            ...
            self.generated_decisions.append((scenario_arg.target_name, root_name, relative.as_posix(), expected_keys))
            return super().generated_file_decision(
                scenario_arg,
                root_name,
                relative,
                path,
    ...
    generated_leftover = recording_oracle.roots["project"] / "leftover/graphify.md"
    generated_leftover.parent.mkdir(parents=True)
    generated_leftover.write_text("generated by graphify\n", encoding="utf-8")

    adapter.universal_uninstall_checks(runner, installed, [])

    assert recording_oracle.generated_decisions == [
        (
            "unit",
            "project",
            "leftover/graphify.md",
