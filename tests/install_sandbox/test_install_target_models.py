...
def test_json_effects_declare_behavior_expectations() -> None:
    for target_name, scope, scenario, entry in scenario_entries():
        if entry.content_kind != "json":
            continue

        assert entry.content_kind == "json"
        assert entry.marker == "graphify"
        assert entry.json_expectation is not None, f"{target_name}/{scope}/{entry.relative}"
        ...
        assert has_hooks != has_plugin, f"{target_name}/{scope}/{entry.relative}"
        if has_plugin:
            ...
            ), f"{target_name}/{scope}/{entry.relative}"


def test_json_schema_names_follow_file_surface_conventions() -> None:
    for target_name, scope, _, entry in scenario_entries():
        if entry.content_kind != "json":
            continue
        ...
        assert entry.json_expectation.schema_name == expected_schema, f"{target_name}/{scope}/{entry.relative}"
...
def test_text_section_user_content_policy_follows_instruction_file_conventions() -> None:
    for target_name, scope, _, entry in scenario_entries():
        ...
        assert entry.text_expectation.preserve_user_content is should_preserve, f"{target_name}/{scope}/{entry.relative}"
        assert entry.text_expectation.require_user_content_on_uninstall is should_preserve, f"{target_name}/{scope}/{entry.relative}"
...
def test_claude_instruction_docs_use_legacy_hash_marker_and_removal_policy() -> None:
    claude_instruction_entries = {
        entry_id(target_name, scope, entry): entry
        for target_name, scope, _, entry in scenario_entries()
    ...
    for (target_name, scope, root, relative), entry in claude_instruction_entries.items():
        assert entry.marker == "# graphify", f"{target_name}/{scope}/{relative}"
        assert not entry.text_expectation.repair_stale_graphify_section
        assert entry.remove_on_uninstall is not (root == "home"), f"{target_name}/{scope}/{relative}"
...
def test_only_home_claude_instruction_docs_are_left_after_uninstall() -> None:
    non_removable_entries = {
        entry_id(target_name, scope, entry)
        for target_name, scope, _, entry in scenario_entries()
    ...
    for target_name in REGISTRY.target_names:
        for scope in ("user", "project"):
            ...
            if scope == "user":
                ...
                ) is (not roots <= {"home"}), f"{target_name}/{scope}"
                ...
                ) is (not roots <= {"project"}), f"{target_name}/{scope}"
...
def test_user_scopes_without_uninstall_commands_declare_public_cli_risk() -> None:
    for target_name in REGISTRY.target_names:
        scenario = REGISTRY.make_scenario(target_name, "user")
        if scenario is None:
            continue

        assert (
            install_target_models.PUBLIC_CLI_LACKS_USER_SKILL_UNINSTALL_NOTE in scenario.risk_notes
        ) is (scenario.uninstall_command is None), target_name


def test_mixed_root_products_are_known_exceptions() -> None:
    mixed_root_scenarios = {
        (target_name, scope): tuple(sorted({entry.root for entry in scenario.expected}))
        for target_name in REGISTRY.target_names
        for scope in ("user", "project")
        if (scenario := REGISTRY.make_scenario(target_name, scope)) is not None
...
def test_simulated_runtime_products_declare_linux_layout_limits() -> None:
    simulated_targets = {
        target_name
        for target_name in REGISTRY.target_names
        if REGISTRY.target_spec(target_name).simulated_linux_layout
    }

    assert simulated_targets == {"antigravity-windows", "windows"}
    for target_name in simulated_targets:
...
def test_agents_target_metadata_and_surfaces_are_generic_standards_facts() -> None:
    spec = REGISTRY.target_spec("agents")
...
def test_non_agents_targets_default_to_product_kind() -> None:
    product_target_kinds = {
        target_name: REGISTRY.target_spec(target_name).target_kind
        for target_name in REGISTRY.target_names
...
def test_vscode_reference_bundle_guard_is_declarative() -> None:
    targets_with_reference_bundles = {
        target_name
        for target_name in REGISTRY.target_names
        if REGISTRY.target_spec(target_name).reference_bundles
    }
    spec = REGISTRY.target_spec("vscode")

    assert targets_with_reference_bundles == {"vscode"}
...
def test_products_with_intentionally_nonstandard_skill_paths_are_known() -> None:
    nonstandard_skill_paths = {
        target_name: (spec.user_skill, spec.project_skill)
        for target_name in REGISTRY.target_names
        if (spec := REGISTRY.target_spec(target_name)).user_skill != f".{target_name}/skills/graphify/SKILL.md"
