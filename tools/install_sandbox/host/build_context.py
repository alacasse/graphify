...
def prepare_context(subject: Path, directory: Path) -> Path:
    ...
    for filename in (
        ...
        shutil.copyfile(sandbox / "image" / filename, context / filename)
    for directory_name in ("contracts", "container"):
        shutil.copytree(
            sandbox / directory_name,
            context / directory_name,
            ignore=shutil.ignore_patterns("__pycache__", "*.pyc", "*.pyo", "graphify-out", ".*"),
        )
    shutil.copytree(
        subject,
        context / "subject",
