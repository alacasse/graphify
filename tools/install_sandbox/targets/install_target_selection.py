...
def unsupported_scope_reason(specs: dict[str, InstallTargetSpec], target_name: str, scope: str) -> str | None:
    return target_spec(specs, target_name).unsupported_scopes.get(scope)


def direct_uninstall_command(specs: dict[str, InstallTargetSpec], target_name: str) -> tuple[str, ...] | None:
    scope = target_spec(specs, target_name).scopes.get("user")
    return None if scope is None else scope.uninstall_command


def generic_install_command(target_name: str, scope: str) -> tuple[str, ...]:
    return _generic_install_command(target_name, scope)
...
def install_variants_for_scope(
    ...
    scope_spec = target_spec(specs, target_name).scopes.get(scope)
...
def direct_install_command(specs: dict[str, InstallTargetSpec], target_name: str, scope: str) -> tuple[str, ...] | None:
    ...
    if scope_spec is None:
        return None
    for variant in install_variants_for_scope(specs, target_name, scope):
        if variant.label == "direct":
            return variant.command
    return None


def make_scenario(specs: dict[str, InstallTargetSpec], target_name: str, scope: str) -> Scenario | None:
    spec = target_spec(specs, target_name)
...
def scenario_id(target_name: str, scope: str) -> str:
    raw = f"{target_name}-{scope}".lower()
    safe = re.sub(r"[^a-z0-9_.-]+", "-", raw)
    safe = re.sub(r"[-_.]{2,}", "-", safe).strip(".-_")
    return safe or "scenario"


def coverage_records(specs: dict[str, InstallTargetSpec], target_names: list[str], scope: str) -> list[dict[str, object]]:
    records: list[dict[str, object]] = []
    for target_name in target_names:
        for one_scope in selected_scopes(scope):
            reason = unsupported_scope_reason(specs, target_name, one_scope)
            scenario = make_scenario(specs, target_name, one_scope) if reason is None else None
                ...
                records.append(
                    {
                        ...
                        "scope": one_scope,
                        "status": "runnable",
                        "scenario_id": scenario_id(target_name, one_scope),
